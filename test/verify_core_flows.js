import { calculateMatchScore, rankCandidatesForProject, findCandidatesForMissingSkill } from '../src/services/matchingEngine.js';
import { calculateSkillLevelFromScore, getSkillLevelLabel, QUESTION_BANKS } from '../src/data/assessmentQuestions.js';
import { CURRENT_USER, SEED_CANDIDATES, INITIAL_PROJECTS } from '../src/data/seedData.js';
...
expectedDomains.forEach(domain => {
  assert(QUESTION_BANKS[domain] && QUESTION_BANKS[domain].length === 5, `Question bank for "${domain}" contains 5 calibrated questions`);
});
...
const maya = SEED_CANDIDATES.find(c => c.name === 'Maya Patel'); // ML L5 (Verified 94%), Python L5 (Verified 96%)
